 ...
 *
 * @return Im Erfolgsfall OSMP_SUCCESS; falls der OSMP_Datatype nicht existiert,
 * OSMP_FAILURE
 */
 ...
 * Zugang zu den gemeinsamen Ressourcen der OSMP-Prozesse. Sie muss von jedem
 * OSMP-Prozess zu Beginn aufgerufen werden. Durch diesen Aufruf wird außerdem
 * der Posteingang des Prozesses freigegeben.
 *
 ...
 * den OSMP-Starter Prozess zurück. Sollte mit der Zahl übereinstimmen, die in
 * der Kommandozeile dem OSMP-Starter übergeben wird.
 *
 ...
 * aufrufenden OSMP-Prozesses von 0,…,np-1 zurück.
 *
 ...
 * sendende Nachricht beginnt im aufrufenden Prozess bei der Adresse buf. Die
 * Funktion ist blockierend, d.h. wenn sie in das aufrufende Programm
 * zurückkehrt, ist der Kopiervorgang abgeschlossen.
 *
 ...
 * des angegebenen Datentyps datatype. Die Nachricht wird an die Adresse buf des
 * aufrufenden Prozesses geschrieben. Unter source wird die OSMP-Prozessnummer
 * des sendenden Prozesses und unter len die tatsächliche Länge der gelesenen
 * Nachricht abgelegt. Die Funktion ist blockierend, d.h. sie wartet, bis eine
 * Nachricht für den Prozess vorhanden ist. Wenn die Funktion zurückkehrt, ist
 * der Kopierprozess abgeschlossen. Die Nachricht gilt nach dem Aufruf dieser
 * Funktion als abgearbeitet.
 *
 * @param [out] buf      Startadresse des Puffers im lokalen Speicher des
 ...
 * Hierbei muss jeder Prozess zuvor alle noch vorhandenen Nachrichten
 * abarbeiten. Dies bedeutet, dass der Posteingang gesperrt wird und alle noch
 * vorhandenen Nachrichten gelöscht werden.
 *
 * @return Im Erfolgsfall OSMP_SUCCESS, sonst OSMP_FAILURE
 */
 ...
 *
 * @param [out]     buf Startadresse des Speicherbereichs, wo die zu empfangende
